...
/**
 * Contains constants for The parent class of all joysticks, allowing for driver input to be easily
 * altered for different devices.
 *
 * @author FRC 1778 Chill Out
 */
class Joystick(private val name: String, private val port: Int) {

    fun isConnected(name: String, port: Int): Boolean {
        return if (DriverStation.getInstance().isDSAttached) {
            name == DriverStation.getInstance().getJoystickName(port) && (DriverStation.getInstance().getStickAxisCount(
                port
            ) > 0 || DriverStation.getInstance().getStickButtonCount(port) > 0 || DriverStation.getInstance().getStickPOVCount(
                port
            ) > 0)
        } else {
            false
